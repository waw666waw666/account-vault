# Account Vault · 账号库

> **我有一堆谷歌号，邮箱记不住，只能靠头像认。所以我做了这个。**

一个**靠头像认账号**的本地账号管理工具。粘贴账号列表截图，它会自动把每个账号的 **头像图片抠出来**、识别出邮箱，整理成一眼就能认出的卡片。

每个账号底下还能挂**独立的备注和多条快捷复制项**（登录口令、备用邮箱、恢复码……），用**标签库**分类，全部数据只存在你自己的浏览器里，不联网、不上传、无后端。

![主界面](docs/screenshots/main-view.png)

<p align="center"><sub>每个账号带真实头像，扫一眼就知道哪个是哪个</sub></p>

![账号网格](docs/screenshots/account-grid.png)

<p align="center"><sub>邮箱脱敏显示，点一下即复制</sub></p>

> **状态**：个人项目，持续迭代中。核心功能可用，欢迎提 Issue。

---

## 它解决什么问题

注册的谷歌号、邮箱号多了以后，真正的麻烦不是"记不住密码"，而是：

- **邮箱名长得都差不多** —— 三四个字母加几个数字，看着都像，根本对不上人
- **密码管理器不认头像** —— Bitwarden、KeePass 存的是文字，你还得逐个点开看
- **截图存了一堆** —— 相册里几十张账号截图，找一个要翻半天

这个工具的核心能力是：**你把截图粘进来，它把头像和邮箱一起抠出来，然后你靠头像认号。**

---

## 核心特性

### 🖼️ 自动抠头像（这是重点）

粘贴截图后，程序会在头像位置逐个采样像素：

- 检测到**颜色杂乱**（超过 10 种色簇、主色占比低于 38%）→ 判定为**真实头像照片** → 直接裁剪下来存为图片
- 检测到**颜色单一** → 判定为**首字母色块** → 提取字母并匹配主色生成占位头像

识别结果的头像还能手动更换：点一下就能传自己的图，或者改字母和颜色。

### 📋 截图 OCR 批量导入

- `Ctrl V` 直接粘贴，无需保存文件
- **全程本地识别**（浏览器内 WASM 跑 tesseract），截图不上传任何服务器
- 自动识别邮箱格式（`xxx@xxx.xxx`），把邮箱和昵称拆开
- 自动检测重复账号，标出"已存在"和"本批重复"
- 识别结果可逐条勾选、修改后再导入

### 🔐 本地加密

- 可选主密码，PBKDF2-SHA256（250,000 次迭代）派生密钥
- AES-256-GCM 加密整个库，盐值随机、每次加密独立 IV
- 数据存于浏览器 IndexedDB，**无服务器、无数据库、无上报**

### 📁 板块分组

按邮箱类型把账号分开管理，各板块有自己的颜色，侧栏一眼可辨：

- QQ / 常用邮箱
- Google 邮箱
- Outlook 邮箱
- 其他邮箱
- 或者你自己新建

板块也能重命名、改色、删除；删除时可以把里面的账号迁移到另一个板块。

### 🗂️ 标签系统

不只是一串字符串，是一个可管理的标签库：

- **独立标签库页面** —— 新建、重命名、改颜色、删除
- **删除有提示** —— 会告诉你「该标签正被 12 个账号使用，删除后将从这些账号移除」
- **一键清理** —— 清掉所有没被使用的空标签
- **搜索覆盖标签** —— 搜关键词能命中标签名
- **卡片上最多显示 2 个** —— 多了折成 `+3`，悬停展开

实际用法比如：`长期号`、`已验证`、`学生会员`、`GPT Plus`、`备用`、`即将到期`。

### 📋 每个账号都有独立的内部空间

一个账号不只是"邮箱 + 密码"，它内部能装：

- **备注** —— 自由文本，记这个号的情况
- **注册时间** —— 支持写 `2021`、`2021年`、`5年老号`，自动换算账号年龄
- **密码 / 2FA 密钥**
- **快捷复制项（无限条）** —— 每条都有自己的名字、内容、图标、颜色、备注

快捷复制项才是真正好用的地方：

| 字段 | 说明 |
|---|---|
| 标签 | 如「登录口令」「备用邮箱」「API Token」 |
| 内容 | 要复制的文本 |
| **图标** | **自动识别**——填「验证码」给数字图标，填「手机」给手机图标，填 `api_key` 给代码图标 |
| **颜色** | 跟随图标自动配色 |
| **敏感保护** | 勾选后在卡片上遮盖，需点眼睛才显示 |
| 备忘说明 | 如「适用场景或过期时间」 |

也就是说，一个账号底下可以挂着：登录密码、PIN 码、备用邮箱、恢复码、订单号——**每项都能单独一键复制，敏感项自动遮盖**。

### 🖱️ 交互细节

- **卡片悬停快速复制** —— 不用点进详情
- **拖拽排序** —— 按你自己的习惯排（dnd-kit）
- **邮箱脱敏** —— 一键隐藏邮箱，防旁人窥屏
- **搜索全字段** —— 名字、邮箱、注册时间、标签、备注一起搜
- **JSON 导入导出** —— 方便备份和换设备

---

## 和其他账号管理器的区别

| | 本工具 | 普通密码管理器 |
|---|---|---|
| 录入方式 | **粘贴截图，自动识别** | 手动逐条输入 |
| 认号方式 | **靠头像图片** | 靠文字搜索 |
| 账号内部 | **可挂多条快捷复制项** | 通常只有账号/密码/网址 |
| 敏感字段 | **逐项标记遮盖** | 整体加密 |
| 标签 | **独立标签库可管理** | 多为简单分组 |
| 存储 | 浏览器本地 | 通常有云同步 |
| 定位 | 多账号快速辨认 | 全量凭据保管 |

> 它**不是** Bitwarden / KeePass 的替代品。它是一个「谷歌号太多、认不过来」场景下的专用工具。

---

## 快速开始

需要 **Node.js 18+**。

```bash
git clone https://github.com/waw666waw666/account-vault.git
cd account-vault
npm install

# 准备 OCR 识别组件（截图导入功能需要，约 4MB，仅首次）
npm run setup:ocr

npm run dev
```

打开 http://127.0.0.1:5188/

### Windows 一键启动

仓库自带 `run.bat`，双击即可。它会自动检查 Node.js、安装依赖并启动服务。

停止服务：双击 `stop.bat`。

---

## 怎么用

1. 打开 http://127.0.0.1:5188/
2. 把账号列表截图**截图到剪贴板**（系统截图工具即可），或准备好图片文件
3. 在页面里按 `Ctrl V` 粘贴
4. 等几秒，本地 OCR 识别完成 —— 每个账号的头像、昵称、邮箱都会列出来
5. 逐条确认（可改头像、改名字、取消勾选不要的），点导入
6. 完成。之后靠头像认号，点一下复制邮箱或密码

> 截图来源可以是谷歌账号切换列表、邮箱客户端列表、任何「头像 + 邮箱」并排的界面。

---

## 命令

| 命令 | 说明 |
| --- | --- |
| `npm run dev` | 启动开发服务器（127.0.0.1:5188） |
| `npm run build` | 类型检查 + 生产构建 |
| `npm run preview` | 预览生产构建 |
| `npm test` | 运行单元测试 |
| `npm run test:watch` | 测试监听模式 |
| `npm run setup:ocr` | 下载 / 复制 OCR 识别组件 |

---

## 关于 OCR 组件

`public/ocr/` 下的三个文件约 8.8MB，**不纳入 Git 仓库**（避免仓库臃肿），需要运行一次：

```bash
npm run setup:ocr
```

该脚本会：

1. 从 `node_modules` **复制** `worker.min.js` 和 `tesseract-core-simd-lstm.wasm.js`（版本与依赖严格一致，无需联网）
2. **下载** `eng.traineddata` 英文语言包（约 4MB，需联网一次）

若网络受限，可手动下载 [eng.traineddata](https://github.com/tesseract-ocr/tessdata_fast/raw/main/eng.traineddata) 放到 `public/ocr/lang/` 目录。

> 未准备 OCR 组件时，除截图导入外的所有功能均可正常使用。

---

## 数据与隐私

**数据存放在浏览器的 IndexedDB 中，与项目目录无关。**

这意味着：

- 清除浏览器数据、更换浏览器或使用无痕模式，**数据会丢失**
- 建议定期通过「设置与备份」导出 JSON
- 项目不含任何埋点、统计或上报逻辑

### 加密说明

开启主密码后，整个数据仓库会以 AES-256-GCM 加密后存储：

- 密钥由 PBKDF2-SHA256 派生，迭代 250,000 次
- 盐值随机生成，每次加密使用独立 IV
- **主密码一旦遗忘无法恢复**，请务必妥善保存

---

## 技术栈

| 层 | 选型 |
| --- | --- |
| 框架 | React 19 |
| 构建 | Vite 8 |
| 语言 | TypeScript 7 |
| 存储 | IndexedDB（无后端） |
| 加密 | Web Crypto API（PBKDF2 + AES-GCM） |
| OCR | tesseract.js 7（WASM，本地运行） |
| 拖拽 | dnd-kit |
| 图标 | lucide-react |
| 测试 | Vitest |

---

## 项目结构

```
src/
  App.tsx                     应用主组件与状态编排
  main.tsx                    入口
  types.ts                    数据模型定义
  storage.ts                  IndexedDB 持久化
  crypto.ts                   加密与解密
  ocr.ts                      截图识别与图像预处理
  bridge.ts                   批量导入指令处理
  utils.ts                    通用工具
  seed.ts                     初始数据
  styles.css                  样式
  components/
    Views.tsx                 主视图（列表 / 板块 / 标签）
    Dialogs.tsx               编辑对话框
    ScreenshotImportDialog.tsx 截图导入
    ConfirmModal.tsx          确认弹窗
    Common.tsx                通用组件
scripts/
  setup-ocr.cjs               准备 OCR 组件
  launcher.ps1                Windows 启动脚本
public/
  ocr/                        OCR 组件（需 setup:ocr 生成）
```

---

## 已知限制

- **OCR 目前仅支持英文识别**（`eng.traineddata`）。识别中文账号备注需要额外语言包
- **Windows 启动脚本为主**。`run.bat` / `stop.bat` 仅适用于 Windows，其他平台请用 `npm run dev`
- **无浏览器扩展形态**，需本地运行开发服务器
- 单用户、单机设计，不含多设备同步

---

## 参与贡献

欢迎提交 Issue 和 Pull Request。

- 提交前请确保 `npm test` 与 `npm run build` 均通过
- 请保持既有代码风格（无分号、单引号、2 空格缩进）

---

## 许可证

[MIT](./LICENSE)
